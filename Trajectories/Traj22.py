...
if __name__=='__main__':
    # Load data
    [u_test,ttest]=pickle.load(open('u22.p','rb'))
    [M,N]=u_test.shape

    # Load encoder
    trunc=8
    model,_,_=buildmodel.buildmodel(N,trunc)
    model.load_weights('model22.h5')
    encode = Model(inputs=model.input,outputs=model.get_layer('Dense_In2').output)

    [u_mean,u_std,U]=pickle.load(open('PCA22.p','rb'))
    a_test=(u_test-u_mean[np.newaxis,:])/u_std[np.newaxis,:]@ U
    h=encode.predict(a_test)

    decode=buildmodel.decode(N,trunc)
    for i in range(2):
    ...
    plt.tight_layout()
    plt.savefig('Pred22.png',bbox_inches="tight")
