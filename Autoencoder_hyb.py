...
if __name__=='__main__':
    ...
    for trunc in range(5,11):
        ...
        ###########################################################################
        # Build model
        model,EPOCHS,optimizer=buildmodel.buildmodel_hybrid(N,U,trunc)
        ...
        history = model.fit(u_train, [u_train,np.zeros((len(u_train),trunc))],epochs=EPOCHS, validation_data=(u_test[:1000:10,:],[u_test[:1000:10,:],np.zeros((len(u_test[:1000:10,:]),trunc))]), verbose=0,callbacks=[PrintDot(),callback])
        ...
        plot_history(hist)
        plt.tight_layout()
        plt.savefig(open('Error'+str(trunc)+'Hyb.png','wb'))
        plt.close()

        [pred_u,_]=model.predict(u_test)    
        ...
        plt.tight_layout()
        plt.savefig(open('Statistics'+str(trunc)+'Hyb.png','wb'))
        plt.close()
        
        # Save models
        model.save_weights('model'+str(trunc)+'Hyb.h5')

        # Save MSE
        pickle.dump([MSE,MSE_PCA],open('MSE'+str(trunc)+'Hyb.p','wb'))
        MSEsig.append(MSE)
    ...
    plt.xlabel(r'$d_h$')
    plt.ylabel('MSE')
    plt.savefig(open('ComparisonHyb.png','wb'))